...
左（スマホでは上）に書くと、右（スマホでは下）に整形後がその場で出ます。
...
- 書いたそばから整形後（AIにはこう見えます）が見える（パソコンは左右、スマホは上が書く欄・下が整形後）
- 「押すと入る」ブロック 11個（見出し・箇条書き・表・太字など）。押すと小さな見本がカーソルの位置に入り、
  見本の文字が選ばれた状態になるので、そのまま打てば置き換わる。白紙から組み上げる
- お手本 9つ（README.md（この練習帳の説明書）／基本の書き方／AIへのお願い文／CLAUDE.md・AGENTS.md／
  アプリ・ホームページ・チラシを作ってもらう／AIフレンドを作る／議事録）
- チャレンジ 7つ（日記／買い物メモ／自己紹介／AIへのお願い／CLAUDE.md／AIフレンド／議事録）。書いた文字が条件を満たすとその場でチェックが付く
- 書いたものをコピー（AIに貼る）、.md で保存、整形後を画像（PNG）・PDF・.html で保存
- 手元の .md ファイル（TODO.md など）を開いて、この画面で直して上書き保存（Chrome / Edge。Ctrl+S でも）。
  ほかのブラウザでは開くだけで、保存は「.md で保存」から
